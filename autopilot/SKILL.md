...
- User wants to review or critique an existing plan -- use `omo-plan --review`
...
1. **Phase 0 - Expansion**: Turn the user's idea into a detailed spec
   - **If ralplan consensus plan exists** (`.opencode/plans/ralplan-*.md` or `.opencode/plans/consensus-*.md` from the 3-stage pipeline): Skip BOTH Phase 0 and Phase 1 — jump directly to Phase 2 (Execution). The plan has already been Planner/Architect/Critic validated.
   - **If deep-interview spec exists** (`.opencode/specs/deep-interview-*.md`): Skip analyst+architect expansion, use the pre-validated spec directly as Phase 0 output. Continue to Phase 1 (Planning).
   ...
   - **Otherwise**: Analyst (`category="ultrabrain"`) extracts requirements, Architect (`category="ultrabrain"`) creates technical specification
   - Output: `.opencode/autopilot/spec.md`
...
2. **Phase 1 - Planning**: Create an implementation plan from the spec
   ...
   - Architect (`category="ultrabrain"`): Create plan (direct mode, no interview)
   - Critic (`category="ultrabrain"`): Validate plan
   - Output: `.opencode/plans/autopilot-impl.md`

3. **Phase 2 - Execution**: Implement the plan using Ralph + Ultrawork
   - Executor (`category="quick"`): Simple tasks
   - Executor (default category): Standard tasks
   - Executor (`category="ultrabrain"`): Complex tasks
...
6. **Phase 5 - Cleanup**: Delete all state files on successful completion
   - Remove `.opencode/state/autopilot-state.json`, `ralph-state.json`, `ultrawork-state.json`, `ultraqa-state.json`
   - Run `/cancel` for clean exit
</Steps>

<Tool_Usage>
- Use `Task(subagent_type="architect", category="ultrabrain")` for Phase 4 architecture validation
- Use `Task(subagent_type="security-reviewer")` for Phase 4 security review
- Use `Task(subagent_type="code-reviewer", category="ultrabrain")` for Phase 4 quality review
...
Why bad: This is an exploration/brainstorming request. Respond conversationally or use the omo-plan skill.
</Bad>
</Examples>
...
## Troubleshooting
**Stuck in a phase?** Check TODO list for blocked tasks, review `.opencode/autopilot-state.json`, or cancel and resume.
...
If a deep-interview spec already exists at `.opencode/specs/deep-interview-*.md`, autopilot uses it directly as Phase 0 output (the spec has already been mathematically validated for clarity).
...
When autopilot detects a ralplan consensus plan (`.opencode/plans/ralplan-*.md` or `.opencode/plans/consensus-*.md`), it skips both Phase 0 (Expansion) and Phase 1 (Planning) because the plan has already been:
