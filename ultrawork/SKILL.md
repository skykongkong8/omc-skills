...
<Why_This_Exists>
Sequential task execution wastes time when tasks are independent. Ultrawork enables firing multiple agents simultaneously and routing each to the right category tier, reducing total execution time while controlling token costs. It is designed as a composable component that ralph and autopilot layer on top of.
</Why_This_Exists>

<Execution_Policy>
- Fire all independent agent calls simultaneously -- never serialize independent work
- Always pass the `category` parameter explicitly when delegating for appropriate model routing
...
3. **Route to correct tiers**:
   - Simple lookups/definitions: `category="quick"`
   - Standard implementation: default category (no category parameter needed)
   - Complex analysis/refactoring: `category="ultrabrain"`
...
<Tool_Usage>
- Use `Task(subagent_type="executor", category="quick")` for simple changes
- Use `Task(subagent_type="executor")` for standard work (default category)
- Use `Task(subagent_type="executor", category="ultrabrain")` for complex work
...
```
Task(subagent_type="executor", category="quick", prompt="Add missing type export for Config interface")
Task(subagent_type="executor", prompt="Implement the /api/users endpoint with validation")
Task(subagent_type="executor", prompt="Add integration tests for the auth middleware")
```
...
```
Task(subagent_type="executor", prompt="npm install && npm run build", run_in_background=true)
Task(subagent_type="executor", category="quick", prompt="Update the README with new API endpoints")
```
...
```
Task(subagent_type="executor", category="ultrabrain", prompt="Add a missing semicolon")
```
Why bad: Ultrabrain category is expensive overkill for a trivial fix. Use `category="quick"` instead.
</Bad>
</Examples>
