...
- User wants to explore or plan before committing -- use `omo-plan` skill instead
...
- Always pass the `category` parameter explicitly when delegating to agents for appropriate model routing
...
1. **PRD Setup** (first iteration only):
   a. Check if `prd.json` exists (in project root or `.opencode/`). If it already exists, read it and proceed to Step 2.
   b. If no `prd.json` exists, the system has auto-generated a scaffold. Read `.opencode/prd.json`.
...
3. **Implement the current story**:
   - Delegate to specialist agents at appropriate tiers:
     - Simple lookups: `category="quick"` -- "What does this function return?"
     - Standard work: default category -- "Add error handling to this module"
     - Complex analysis: `category="ultrabrain"` -- "Debug this race condition"
...
7. **Reviewer verification** (tiered, against acceptance criteria):
   - <5 files, <100 lines with full tests: STANDARD tier minimum (`Task(subagent_type="architect")`)
   - Standard changes: STANDARD tier (`Task(subagent_type="architect")`)
   - >20 files or security/architectural changes: THOROUGH tier (`Task(subagent_type="architect", category="ultrabrain")`)
   - If `--critic=critic`, use the `critic` agent for the approval pass
   - If `--critic=codex`, run `omo ask codex --agent-prompt critic "..."` for the approval pass. The Codex critic prompt MUST include:
...
7.5 **Mandatory Deslop Pass** (runs unconditionally after Step 7 approval, unless `{{PROMPT}}` contains `--no-deslop`):
   ...
   - **ai-slop-cleaner is a SKILL, not an agent.** Do NOT call it via `Task(subagent_type="ai-slop-cleaner")` — that subagent type does not exist and the call will fail with "Agent type not found". If you see that error, retry with the Skill tool — do NOT substitute a similarly-named agent like `code-simplifier` as a "closest match".
...
8. **On approval**: After Step 7.6 passes (with Step 7.5 completed, or skipped via `--no-deslop`), run `/cancel` to cleanly exit and clean up all state files
...
- Use `Task(subagent_type="architect", category="ultrabrain")` for architect verification cross-checks when changes are security-sensitive, architectural, or involve complex multi-system integration
- Use `Task(subagent_type="critic", category="ultrabrain")` when `--critic=critic`
- Use `omo ask codex --agent-prompt critic "..."` when `--critic=codex`. Construct the prompt to include: (a) prd.json acceptance criteria, (b) files changed + related files, (c) explicit optimality question: "Is there a meaningfully simpler, faster, or more maintainable approach that achieves the same acceptance criteria?"
...
- **Skill vs agent invocation**: `ai-slop-cleaner` is a skill, invoke via `Skill("ai-slop-cleaner")`. `architect`, `critic`, `executor` etc. are agents, invoke via `Task(subagent_type="<name>")`. If you ever get "Agent type ... not found" for an identifier, the item may be a skill — retry with the Skill tool. Do NOT substitute a similarly-named agent as a "closest match".
</Tool_Usage>
...
```
Task(subagent_type="executor", category="quick", prompt="Add type export for UserConfig")
Task(subagent_type="executor", prompt="Implement the caching layer for API responses")
Task(subagent_type="executor", category="ultrabrain", prompt="Refactor auth module to support OAuth2 flow")
```
